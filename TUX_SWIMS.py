...
nullImg = pygame.image.load('null.gif')
einsImg = pygame.image.load('eins.gif')
eatenImg = pygame.image.load('eaten.gif')
...
def crash(xcrash,ycrash):
    gameDisplay.blit(crashImg,(xcrash,ycrash))
...
def game_loop():
    ...
    thing_startx = 1920
    thing_starty = random.randrange(0, display_height-100)
    thing_speed =  -12
    thing_width = 150
    thing_height = 100
    ...
    bx = 1920
    by = random.randrange(0, display_height-100)
    b_speed =  -6
    b_width = 100
    b_height = 100
...
#Hintergrund:
        ...
        if cx > display_width:
            cx = 0 + c_width
            cy = random.randrange(0,display_height-100)
            dodged += 1
            c_speed += 1
            c_height += (dodged * 1.2)
        ...
        binary = ("{0:b}".format(fish))
        print(binary)
        text = str(binary)
        position = list(text)
        print(position)

        gameDisplay.blit(eatenImg,(xnull-50,ynull))
        gameDisplay.blit(zahl,(xnull,ynull))
        if position[0] == '1':
            print("position")
            zahl = einsImg
        if position[0] == '0':
            zahl = nullImg
        if fish > 1:
            gameDisplay.blit(zahl1,(xnull+25,ynull))
            if position[1] == '1':
                print("position")
                zahl1 = einsImg
            if position[1] == '0':
                zahl1 = nullImg
        if fish > 3:
            gameDisplay.blit(zahl2,(xnull+50,ynull))
            if position[2] == '1':
                print("position")
                zahl2 = einsImg
            if position[2] == '0':
                zahl2 = nullImg
        if fish > 7:
            gameDisplay.blit(zahl3,(xnull+75,ynull))
            if position[3] == '1':
                print("position")
                zahl3 = einsImg
            if position[3] == '0':
                zahl3 = nullImg
        if fish > 15:
            gameDisplay.blit(zahl4,(xnull+100,ynull))
            if position[4] == '1':
                print("position")
                zahl4 = einsImg
            if position[4] == '0':
